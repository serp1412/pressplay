	...
	module Generator
		class Framework
			...
			def self.generate_for(project, dir = Dir.getwd)
				...
				add_info_plist(framework_group, app_build_settings, dir)
        @app_target.add_dependency(@framework_target)
        add_framework_to_embed_binaries
			...
			def self.add_info_plist(group, app_build_settings, dir)
        ...
        group.new_file(relative_path_string)
			end

			def self.add_framework_to_embed_binaries
        ...
        embed_build_phase = embed_binaries_phase(@app_target)
        build_file = embed_build_phase.add_file_reference(file_ref)
        build_file.settings = { 'ATTRIBUTES' => ['CodeSignOnCopy', 'RemoveHeadersOnCopy'] }
			end

			def self.embed_binaries_phase(target)
