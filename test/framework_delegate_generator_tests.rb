...
public class FrameworkDelegate: UIResponder, UIApplicationDelegate {
		...
		data = PressPlay::Generator::FrameworkDelegate.new.generateFrom(ast, app_delegate)

		assert_equal app_delegate, data.appDelegateRaw
		assert_equal result, data.frameworkDelegateRaw
	end
...
let a = "1"
		...
		puts data.frameworkDelegateRaw
		puts result
